...
def solverSCP(id, mh, maxIter, pop, instancia, DS, repairType, param):
    ...
    results.write(
        f'iteracion,fitness,mejor iteracion,peor iteracion,optimo\n'
    )
    ...
    for iter in range(0, maxIter):
        ...
        if mh == "RSA":
            poblacion = iterarRSA(maxIter, iter, instance.getColumns(), poblacion.tolist(), Best.tolist())
        ...
        results.write(
            f'{iter+1},{str(round(BestFitness))},{str(round(fitness[solutionsRanking[0]]))},{str(round(fitness[solutionsRanking[pop-1]]))},{str(instance.getOptimum())}\n'
        )
